...
$tipopermissao = new tipospermissoes();

if ( isset($_POST['nome']) ){
	
	//verifica se o su possue permissão para alterar(1) no modulo(2)
	if(!permissoesDAO::temPermissao(2,1,$usuario->id_tipo_usuario)){	
		header("Location: ../index.php");
		exit();
	}		
	
	//atribui o id ao objeto tipopermissao
	$tipopermissao->id = addslashes($_POST['id']);
	$tipopermissao->nome = addslashes($_POST['nome']);
	$tipopermissao->descricao = addslashes($_POST['descricao']);
	$tipopermissao->status = 1;
	
	//verifica se o o nome da esta em branco
	if($tipopermissao->nome != ""){
	
	//verifica se o id recebido aponta para um novo usuário(0) ou um antigo(!=0)
	if ($tipopermissao->id != 0){
		$tipopermissao = tipospermissoesDAO::findByPk($tipopermissao->id);
	}			
	$tipopermissao->nome = addslashes($_POST['nome']);
	$tipopermissao->descricao = addslashes($_POST['descricao']);
	$tipopermissao->status = 1;
	$id = tipospermissoesDAO::save($tipopermissao);
	header("Location: home.php");
	exit();	
	
	}else{
		$msg = "O nome do tipo de Permissão não pode estar vazio";
	}		
}  

//verifica se a pagina recebeu um id para alteração
if(isset($_GET['id'])){
	$tipopermissao = tipospermissoesDAO::findByPk($_GET['id']);
	$ep = 0;
}else{
	//verifica se o su possue permissão para inserir(2) no modulo(2)
	if(!permissoesDAO::temPermissao(2,2,$usuario->id_tipo_usuario)){
		header("Location: ../index.php");
		exit();
	}
}
      ...
      <table width="545" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td width="481" bgcolor="#FFFFFF"><!-- texto sempre dentro da tag "<p>" -->
            ...
            <br />
            <br />
			<form action="adicionar.php" method="post" onSubmit="javascript:return confirmaTipoUser(this)" name="tipopermissao" id="tipopermissao">
			  <div align="center">
			  <font class="warning">
			    <?php  if(isset($msg) )
						echo $msg;?>
			  ...
			  <table cellpadding="0" cellspacing="0" width="96%" class="tabela1">
                ...
                <tr>
                  <td><table cellpadding="1" cellspacing="1" width="100%">
                      ...
                      <tr>
						...
						<td class="tabela1_linha2" ><input type="text" class="FORMULARIO"  name="nome" size="10" maxlength="10" value="<?php=stripslashes($tipopermissao->nome)?>"></td>
					  </tr>
					  <tr>
                        <td class="tabela1_linha2"  align="right"> Descrição </td>
					    <td class="tabela1_linha2" ><textarea name="descricao" cols="50" rows="2" class="FORMULARIO" id="descricao"><?php=stripslashes($tipopermissao->descricao)?></textarea></td>
				      </tr>
					  <tr>
			   ...
			   <input type="hidden"  name="id" value="<?php=stripslashes($tipopermissao->id)?>" />
		    </form> 
              <br />
              </p>
          </td>
