...
//verifica se o su possue permissão para excluir(3) no modulo(2)
if(!permissoesDAO::temPermissao(2,3,$usuario->id_tipo_usuario)){
...
if ( isset($_POST['id']) && is_numeric($_POST['id']) )
	tipospermissoesDAO::delete($_POST['id']);
...
if(!isset($nxt)||($nxt<0))
	...
	$total = tipospermissoesDAO::countAll();
	

    $tiposPermissoes = tipospermissoesDAO::findTop($nxt,$total_mural);
                  ...
                  <td><table cellpadding="1" cellspacing="1" width="100%">
                      ...
                      <tr>
                        <td class="tabela1_linha2" nowrap="nowrap"><a href="#">
                          <?php=$tiposPermissoes[$i]->id?>
                        </a></td>
                        <td class="tabela1_linha2" nowrap="nowrap"><a href="#">
                          <?php=$tiposPermissoes[$i]->nome?>
                        </a></td>
                        <td nowrap="nowrap" class="tabela1_linha2"><a href="#">
                          <?php=substr($tiposPermissoes[$i]->descricao, 0, 50)?>
                          <?php  if(strlen($tiposPermissoes[$i]->descricao) >= 50){?>
                          ...
                          <?php  }?>
                        </a></td>
                        <td class="tabela1_linha2" nowrap="nowrap"> <form action="excluir.php" method="post" onSubmit="javascript:return confirma('<?php=$tiposPermissoes[$i]->nome?>','Tipos de Permissões')"><input type="image" src="../images/xis.jpg" width="20" height="21" border="0" /><input type="hidden" value="<?php=$tiposPermissoes[$i]->id?>" name="id" /></form></td>
                      </tr>
					  	 <?php 
							}
						?>
