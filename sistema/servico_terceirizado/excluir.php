...
//verifica se o su possue permissão para excluir(3) no modulo(24)
if(!permissoesDAO::temPermissao(24,3,$usuario->id_tipo_usuario)){
...
if ( isset($_POST['id']) && is_numeric($_POST['id']) ){	
	...
	//verifica se exisite um arquivo de nota na pasta do condomínio
	if($servico->copia_nota != ""){
		ftp::deletanota($servico->copia_nota, $id_condominio); //deleta nota antiga
	}
	servicosTerceirizadosDAO::delete($_POST['id']);
}
      ...
      <table width="545" border="0" cellspacing="0" cellpadding="0">
        <tr>
         ...
         <br />
            ...
            <table cellpadding="0" cellspacing="0" width="99%" class="tabela1">
			   <tr>
                  <td bgcolor="#6598CB"><table width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="89%" class="tabela1_titulo1">Serviços Terceirizados do mês (<?php=$mes?>/<?php=$ano?>)</td>
                ...
                <tr>
				  ...
				  <?php 
					...
					?>
					 <tr>
                        <th colspan="5" class="tabela1_titulo2" scope="col"><div align="left"><strong><?php=$servicos[$i]->tipoServico->nome ?></strong></div></th>
                     </tr>
                    <tr>
                      ...
                      <td width="21%" class="tabela1_titulo2">&nbsp;</td>
			...
			?>	
                    <tr>
                      <td class="<?php=$classe?>"><a href="../despesa/adicionar.php?id=<?php=$servicos[$i]->despesa->id?>" onmouseover="doTooltip(event, 0, '<?php=$servicos[$i]->despesa->documento?>')" onmouseout="hideTip()"><?php=substr($servicos[$i]->despesa->documento,0,6)?>...</a></td>
                      ...
                      <td class="<?php=$classe?>" nowrap="nowrap"><a href="#"><?php=number_format(stripslashes($servicos[$i]->despesa->valor), 2, ',', '.')?></a></td>					  
                      ...
                      <td class="<?php=$classe?>"><form action="excluir.php" method="post" onsubmit="javascript:return confirma('<?php=$servicos[$i]->despesa->documento?>','Serviço Terceirizado')">
                        <input name="image22" type="image" src="../images/xis.jpg" width="20" height="21" border="0" />
                        <input type="hidden" value="<?php=$servicos[$i]->id?>" name="id" />
                      </form></td>
                    </tr>
					<?php  if($servicos[($i+1)]->tipoServico->nome != $tipoServicoAtual){ ?>
                    <tr>
                      <td colspan="2" nowrap="nowrap" class="tabela1_linha2"><a href="#"><div align="right">TOTAL------------&gt;</div></a></td>
