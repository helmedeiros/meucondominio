...
$tipofuncionario = new tiposfuncionarios();

if ( isset($_POST['nome']) ){
	
	//verifica se o su possue permissão para alterar(1) no modulo(21)
	if(!permissoesDAO::temPermissao(21,1,$usuario->id_tipo_usuario)){	
		header("Location: ../index.php");
		exit();
	}		
	
	//atribui o id ao objeto tipofuncionario
	$tipofuncionario->id = addslashes($_POST['id']);
	$tipofuncionario->nome = addslashes($_POST['nome']);
	$tipofuncionario->descricao = addslashes($_POST['descricao']);
	$tipofuncionario->status = addslashes($_POST['status']);;
	
	//verifica se o o nome da esta em branco
	if($tipofuncionario->nome != ""){
	
	//verifica se o id recebido aponta para um novo usuário(0) ou um antigo(!=0)
	if ($tipofuncionario->id != 0){
		$tipofuncionario = tiposfuncionariosDAO::findByPk($tipofuncionario->id);
	}			
	$tipofuncionario->nome = addslashes($_POST['nome']);
	$tipofuncionario->descricao = addslashes($_POST['descricao']);
	$tipofuncionario->status = addslashes($_POST['status']);;
	$id = tiposfuncionariosDAO::save($tipofuncionario);
	header("Location: home.php");
	exit();		
	
	}else{
		$msg = "O nome do tipo de funcionário não pode estar vazio";
	}	
}  

//verifica se a pagina recebeu um id para alteração
if(isset($_GET['id'])){
	$tipofuncionario = tiposfuncionariosDAO::findByPk($_GET['id']);
	$ep = 0;
}else{
	//verifica se o su possue permissão para inserir(2) no modulo(21)
	if(!permissoesDAO::temPermissao(21,2,$usuario->id_tipo_usuario)){
		header("Location: ../index.php");
		exit();
	}
}
    ...
    <td colspan="7" rowspan="9" valign="top" background="../images/bg_geral_box.jpg" bgcolor="#FFFFFF"><table width="545" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td height="71" background="../images/topo_box.jpg"><!-- titulo da secao -->
          <h1>TIPOS DE FUNCIONÁRIOS </h1></td>
      </tr>
    </table>
      <table width="545" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td width="481" bgcolor="#FFFFFF"><!-- texto sempre dentro da tag "<p>" -->
            ...
            <br />
            <br />
			<form action="adicionar.php" method="post" onSubmit="javascript:return confirmaTipoUser(this)" name="tipofuncionario" id="tipofuncionario">
			  <div align="center">
			  <font class="warning">
			    <?php  if(isset($msg) )
						echo $msg;?>
                ...
                <tr>
                  <td><table cellpadding="1" cellspacing="1" width="100%">
                      ...
                      <tr>
						...
						<td class="tabela1_linha2" ><input type="text" class="FORMULARIO"  name="nome" size="50" maxlength="100" value="<?php=stripslashes($tipofuncionario->nome)?>"></td>
					  </tr>
					  <tr>
                        <td class="tabela1_linha2"  align="right"> Descrição </td>
					    <td class="tabela1_linha2" ><textarea name="descricao" cols="50" rows="2" class="FORMULARIO" id="descricao"><?php=stripslashes($tipofuncionario->descricao)?></textarea></td>
				      </tr>
					  <tr>
					    <td  align="right" class="tabela1_linha2">Situação</td>
				        <td  align="right" class="tabela1_linha2"><div align="left">Ativo
				          <input name="status" type="radio" value="1"  <?php  if ( $tipofuncionario->status == 1) { ?> checked="checked" <?php  }?>/>
				          Inativo
  <input name="status" type="radio" value="0"   <?php  if ( $tipofuncionario->status == 0) { ?>checked="checked"<?php  }?> />
                        </div></td>
					  </tr>
					  <tr>
			   ...
			   <input type="hidden"  name="id" value="<?php=stripslashes($tipofuncionario->id)?>" />
		    </form> 
              <br />
              </p>
          </td>
