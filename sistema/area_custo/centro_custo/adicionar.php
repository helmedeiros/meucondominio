...
if (isset($_GET['area']))
 $areacustos = areacustosDAO::findByPk($area);
 
$centrocustos = new centrocustos();

if (isset($_POST['nome'])){
//verifica se o su possue permissão para alterar(1) em areas de custo(30)
	if(!permissoesDAO::temPermissao(26,1,$usuario->id_tipo_usuario)){	
		header("Location: ../../index.php");
		exit();
	}		
	
	//atribui o id ao objeto modulo
	$centrocustos->id = addslashes($_POST['id']);
	$centrocustos->nome = addslashes($_POST['nome']);
    $centrocustos->numero = addslashes($_POST['numero']);
	$centrocustos->id_area_custos = $_POST['area'];
	
	//verifica se o o nome da esta em branco
	if($centrocustos->nome != ""){
	
	
		//verifica se o id recebido aponta para um novo usuário(0) ou um antigo(!=0)
	   	if ($centrocustos->id != 0){
			$centrocustos = centrocustosDAO::findByPk($centrocustos->id, $_POST['area']);
		    $centrocustos->nome = addslashes($_POST['nome']);
	    	$centrocustos->numero = addslashes($_POST['numero']);
	    	$centrocustos->id_area_custos = $area;
	    	$centrocustos->status = 1;
			$id = centrocustosDAO::save($centrocustos);
			header("Location: home.php?area=$area");
			exit(); 
		}else{
			if(!centrocustosDAO::existeByNumNomAr($_POST['nome'], $_POST['numero'], $_POST['area'])){
				$centrocustos->nome = addslashes($_POST['nome']);
				$centrocustos->numero = addslashes($_POST['numero']);
				$centrocustos->id_area_custos = $_POST['area'];
				$centrocustos->status = 1;
				...
				header("Location: home.php?msg=Impossível Adicionar: Centro de Custo já cadastrado&area=$area");
				exit();
			}
		}
	}else{
		$msg = "O nome do centro de custo não pode estar vazio";
	}
} 

//verifica se a pagina recebeu um id para alteração
if(isset($_GET['id'])){
	$centrocustos = centrocustosDAO::findByPk($_GET['id'], $_GET['area']);
	$ep = 0;
}else{
	//verifica se o su possue permissão para inserir(2) em area custo(30)
	if(!permissoesDAO::temPermissao(26,2,$usuario->id_tipo_usuario)){
		header("Location: ../../index.php");
		exit();
	}
}
        ...
        <td><!-- menu superior -->
            <table width="545" border="0" align="center" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td valign="top" background="images/topo_espaco.jpg"><a href="home.php?area=<?php=$area?>"><img src="../../images/botao_listar_off.jpg" name="listar1" width="78" height="40" border="0" id="listar1" /></a></td>
				<td valign="top" background="images/topo_espaco.jpg"><a href="buscar.php?area=<?php=$area?>"><img src="../../images/botao_pesquisar_off.jpg" name="pesquisar" width="90" height="40" border="0" id="listar1" /></a></td>
                <td valign="top" background="images/topo_espaco.jpg"><a href="adicionar.php?area=<?php=$area?>"><img src="../../images/botao_cadastrar.jpg" name="cadastrar1" width="87" height="40" border="0" id="cadastrar1" /></a></td>
                <td valign="top" background="images/topo_espaco.jpg"><a href="#"></a></td>
                <td valign="top" background="images/topo_espaco.jpg"><a href="excluir.php?area=<?php=$area?>"><img src="../../images/botao_excluir_off.jpg" name="excluir1" width="87" height="40" border="0" id="excluir1" /></a></td>
              </tr>
          </table></td>
      </tr>
      <tr>
        <td height="71" background="../../images/topo_box.jpg"><!-- titulo da secao -->
          <h1>CENTROS DE CUSTO</h1></td>
      </tr>
    </table>
      <table width="545" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td width="481" bgcolor="#FFFFFF"><!-- texto sempre dentro da tag "<p>" -->
            ...
            <br />
            <br />
			<form action="adicionar.php" method="post" onSubmit="javascript:return confirmaCentroCusto(this)" nome="centrocusto" numero="centrocusto">
			  <div align="center">
			  <font class="warning">
			    <?php  if(isset($msg) )
						echo $msg;?>
			  ...
			  <table cellpadding="0" cellspacing="0" width="96%" class="tabela1">
                ...
                <tr>
                  <td><table cellpadding="1" cellspacing="1" width="100%">
                      <tr>
                        <td class="tabela1_titulo2" colspan="2"> obs: O Centro de Custo ser&aacute; adicionado na &Aacute;rea de Custo :
                        <?php=$areacustos->nome;?></td>
                      </tr>
                      <tr>
					  	<td class="tabela1_linha2"  align="right"> Nome</td>
						<td class="tabela1_linha2" ><input type="text" class="FORMULARIO"  name="nome" size="30" maxlength="30" value="<?php=stripslashes($centrocustos->nome)?>"></td>
					  </tr>
					  <tr>
                        <td class="tabela1_linha2"  align="right"> Numero </td>
					    <td class="tabela1_linha2" ><input type="text" class="FORMULARIO"  name="numero" size="30" maxlength="30" value="<?php=stripslashes($centrocustos->numero)?>" onKeyPress="return Numero(event);"/></td>
					  </tr>
					  <tr>
			   ...
			   <input type="hidden"  name="id" value="<?php=stripslashes($centrocustos->id)?>" />
