...
if ( isset($area) && is_numeric($area) && isset($objeto) && is_numeric($objeto)){
	...
	$funcionamento = funcionamentoDAO::findByPk($objetos->funcionamento);
}

$membros = membroscondominioDAO::findTopByBusca("", "", $condominio->id);


//verifica se o objeto de lazer esta inativo ou se o funcionamento deste esta impedido
if($objetos->status == 0 or $funcionamento->impedimento == 1){
	$impede = 1;
	$msg = "As reservas neste objeto de lazer estão impedidas pois o objeto encontra-se excluído da base ativa";
	if($funcionamento->impedimento == 1){
		$msg = "As reservas neste objeto de lazer estão impedidas pois o objeto encontra-se no estado de $funcionamento->nome";
	}
}
      ...
      <table width="545" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td width="481" bgcolor="#FFFFFF"><!-- texto sempre dentro da tag "<p>" -->
		    ...
		    ?>
			 <form action="<?php  if(!$impede){?>confirma.php <?php  }else{?>#<?php  }?>" method="post"> 
            <table width="96%" cellpadding="0" cellspacing="0" class="tabela1">
                ...
                <tr>
                  <td><table cellpadding="1" cellspacing="1" width="100%">
                    <tr>
                      <td width="14%" class="tabela1_titulo2">Horário </td>
                      <td width="23%" class="tabela1_titulo2" align="center">STATUS</td>
                      <td width="63%" class="tabela1_titulo2" align="center">CONDÔMINO/APTº</td>
                      </tr>
                    <?php 					
					...
					while($inicio < $fim){		
						...
						$reserva = reservaDAO::findTopByBusca($objetos->id, "", "", $dataInicio, "");							
					?>
                    <tr>
                    	<td class="<?php=$classe?>" nowrap="nowrap"><a href="#"><?php=$data?></a></td>
                      	<td class="<?php=$classe?>" nowrap="nowrap" align="center"><a href="#">
                      	  <input name="<?php='marcar['.$j.'][inicio]'?>" <?php  if($reserva){?> checked="checked"  <?php  }?>type="checkbox" <?php  if(!$libera or $reserva){ ?> disabled="disabled" <?php  }?> class="style3" value="<?php=$data?>" />						  
                      	</a></td>
                   	  	<td class="<?php=$classe?>" align="center">
						<select name="<?php='membro['.$j.']'?>" <?php  if(!$libera or $reserva){ ?> disabled="disabled" <?php  }?>>
							<?php  for($i = 0; $i < sizeof($membros); $i++ ){?>
								<option value="<?php=$membros[$i]->id?>" <?php  if($reserva[0]->id_membro == $membros[$i]->id){?> selected="selected" <?php  }?> > <?php=$membros[$i]->nome?></option>
							<?php  }?>
						</select>				</td>
               	  	  </tr>
                    <?php 
					...
					?>					
							<input name="<?php='marcar['.$j.'][fim]'?>" type="hidden" value="<?php=$data?>" />
					<?php 
							$j += 1;
						}
					?>
			...
			<?php  if(!$impede){?>
						...
						<?php  	if($libera){?>
						<input type="image" src="../../../images/enviar.jpg">
						<?php  }?>
		<?php  }?>
			   </div>
			<?php 
