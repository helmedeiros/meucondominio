              ...
              <table width="459" cellpadding="0" cellspacing="0" class="tabela1">
                <tr>
                  <td bgcolor="#6598CB" colspan="2"><table  cellpadding="0" cellspacing="0">
                    ...
                    <td colspan="2" class="tabela1_linha2">A(s) reserva(s) da(o) <?php=stripslashes($objetos->nome)?> para o dia <?php=$_POST['dia']."/".$_POST['mes']."/".$_POST['ano']?> , nos hor&aacute;rios para os Usuários foi realizada com sucesso:
				  <br />
				  <br />
				  <p>                     
                    <?php  	$horarios = $_POST['horarios'];
					   	for($i = 0; $i < sizeof($horarios); $i++){ 							
							$reserva = new Reserva();
							$reserva->id_membro = $horarios[$i][membro];
							$reserva->id_objeto = $objetos->id;
							$reserva->data_inicio = htmlentities($_POST['ano'])."-".htmlentities($_POST['mes'])."-".htmlentities($_POST['dia']." ".$horarios[$i][inicio]);
							$reserva->data_fim = htmlentities($_POST['ano'])."-".htmlentities($_POST['mes'])."-".htmlentities($_POST['dia']." ".$horarios[$i][fim]);	
							$reserva->comentario = "";
							$reserva->status = 1;					
					?>
							<?php=$reserva->data_inicio?> - <?php=$reserva->id_membro?><br />
					<?php 				
