...
if (isset($_POST['gravaaditivo'])) {
 if ($_POST['aditivo'] == "" ){ $msg = $msg."/ A descrição deve ser completa /";
        header("Location: adicionar.php?reg=$reg");
		exit(); 
		}
...
//atribui o id ao objeto modulo
	$aditivos->id = addslashes($_POST['id']);
	//verifica se o id recebido aponta para um novo usuário(0) ou um antigo(!=0)
	if ($aditivos->id != 0){
    $aditivos = aditivosDAO::findByPk($_POST['id']);}
	$aditivos->id_regimento = $reg;
	$aditivos->data_envio = addslashes($_POST['data']);
    $aditivos->aditivo = addslashes($_POST['aditivo']);
	$regimentos->nome_arquivo = "";
	$id = aditivosDAO::save($aditivos); 
	$lid = aditivosDAO::lastid();
	
	header("Location: adicionar.php?adt=$lid&reg=$reg");
	exit();	
...
<table width="714" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <td><!-- menu superior -->
              <table width="545" border="0" align="center" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td valign="top" background="images/topo_espaco.jpg"><a href="adicionar.php?reg=<?php=$reg?>"><img src="../../images/botao_cadastrar.jpg" name="cadastrar1" width="87" height="40" border="0" id="cadastrar1" /></a></td>
                  <td valign="top" background="images/topo_espaco.jpg"><a href="#"></a></td>
                  <td valign="top" background="images/topo_espaco.jpg"><a href="excluir.php?reg=<?php=$reg?>"><img src="../../images/botao_excluir_off.jpg" name="excluir1" width="87" height="40" border="0" id="excluir1" /></a></td>
                </tr>
            </table></td>
      </tr>
      <tr>
          ...
          <tr>
            ...
            <td width="481" bgcolor="#FFFFFF"><!-- texto sempre dentro da tag "<p>" -->
                <p align="center"> <br />
                    ...
                    <br />
                    <br />
					<?php  if (isset($_GET['adt'])) { ?>
					    <br />
					    </MM:DECORATION></MM_HIDDENREGION>
                </p>
                <form action="adicionarAdt.php?adt=<?php=$adt?>" type="file" method="post" enctype="multipart/form-data">
                  <table cellpadding="0" cellspacing="0" width="96%" class="tabela1">
				   ...
				   <input type="hidden"  name="reg" value="<?php=$reg?>"/>
                </form>
				<?php  } ?>
                <p>
                  ...
                  <?php  if ((!isset($_POST['data']) or isset($msg)) && !isset($_GET['adt'])) { ?>
                </p>
                <form action="adicionar.php?reg=<?php=$reg?>" method="post">
                ...
                <table cellpadding="0" cellspacing="0" width="96%" class="tabela1">
                    ...
                    <tr>
                      <td><table cellpadding="1" cellspacing="1" width="100%">
                        ...
                        <tr>
                          <td width="28%"  align="right" class="tabela1_linha2">Data do Aditivo </td>
                          <td width="72%" class="tabela1_linha2" ><script>DateInput('data', true, 'YYYY-MM-DD')</script>                          </td>
                        </tr>
                        <tr>
                          <td class="tabela1_linha2"  align="right">Descrição</td>
                          <td class="tabela1_linha2" ><textarea name="aditivo" cols="47" rows="5" class="FORMULARIO" ><?php=$aditivos->aditivo?></textarea></td>
                        </tr>
  ...
  <tr>
    <td colspan="2"  align="right" class="tabela1_linha2"><div align="center"><br />
            <input name="gravaaditivo" type="image" src="../../images/enviar.jpg" value="bla"/>
    </div></td>
  </tr>
				 ...
				 <?php  } ?>  
                <input type="hidden"  name="id" value="<?php=stripslashes($aditivos->id)?>" />
                </form>
                <form name="part" action="adicionar.php" method="post" onsubmit="javascript:return confirmaReunioes(this)">
                  <input type="hidden"  name="id" value="<?php=$id?>"/>
                  <input type="hidden"  name="aditivo" value="<?php=$aditivo?>"/>
