...
if ( isset($_POST['nome']) ){
	...
	//verifica se o su possue permissão para alterar(1) no modulo(4)
	if(!permissoesDAO::temPermissao(4,1,$usuario->id_tipo_usuario)){	
		header("Location: ../index.php");
		exit();
	}		
	
	//atribui o id ao objeto funcionamento
	$funcionamento->id = addslashes($_POST['id']);
	$funcionamento->nome = addslashes($_POST['nome']);
	$funcionamento->impedimento = addslashes($_POST['impedimento']);
	$funcionamento->status = addslashes($_POST['status']);
	
	//verifica se o o nome da esta em branco
	if($funcionamento->nome != ""){
	
	//verifica se o id recebido aponta para um novo usuário(0) ou um antigo(!=0)
	if ($funcionamento->id != 0){
		$funcionamento = funcionamentoDAO::findByPk($funcionamento->id);
	}			
	$funcionamento->nome = addslashes($_POST['nome']);
	$funcionamento->impedimento = addslashes($_POST['impedimento']);
	$funcionamento->status = addslashes($_POST['status']);
	$id = funcionamentoDAO::save($funcionamento);
	header("Location: home.php");
	exit();		
	}else{
		$msg = "O nome da situação de funcionamento não pode estar vazio";
	}
}  

//verifica se a pagina recebeu um id para alteração
if(isset($_GET['id'])){
	$funcionamento = funcionamentoDAO::findByPk($_GET['id']);
	$ep = 0;
}else{
	//verifica se o su possue permissão para inserir(2) no modulo(4)
	if(!permissoesDAO::temPermissao(4,2,$usuario->id_tipo_usuario)){
		header("Location: ../index.php");
		exit();
	}
}
    ...
    <td colspan="7" rowspan="9" valign="top" background="../images/bg_geral_box.jpg" bgcolor="#FFFFFF"><table width="545" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td height="71" background="../images/topo_box.jpg"><!-- titulo da secao -->
          <h1>SITUAÇÃO DE FUNCIONAMENTO </h1></td>
      </tr>
    </table>
      <table width="545" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td width="481" bgcolor="#FFFFFF"><!-- texto sempre dentro da tag "<p>" -->
            ...
            <br />
            <br />
			<form action="adicionar.php" method="post" onSubmit="javascript:return confirmaSituacaoFuncionamento(this)" name="funcionamento" id="funcionamento">
			  <div align="center">
			  <font class="warning">
			    <?php  if(isset($msg) )
						echo $msg;?>
			  ...
			  <table cellpadding="0" cellspacing="0" width="96%" class="tabela1">
                ...
                <tr>
                  <td><table cellpadding="1" cellspacing="1" width="100%">
                      ...
                      <tr>
					  	<td width="36%"  align="right" class="tabela1_linha2"> Nome</td>
						<td width="64%" class="tabela1_linha2" ><input type="text" class="FORMULARIO"  name="nome" size="18" maxlength="18" value="<?php=stripslashes($funcionamento->nome)?>"></td>
					  </tr>
					  <tr>
                        <td class="tabela1_linha2"  align="right">Impede o uso do recurso </td>
					    <td class="tabela1_linha2" >Sim
					      <input name="impedimento" type="radio" value="1" <?php  if ( $funcionamento->impedimento == 1) { ?> checked="checked" <?php  }?> />
					      Não
					      <input name="impedimento" type="radio" value="0"  <?php  if ( $funcionamento->impedimento == 0) { ?> checked="checked" <?php  } ?>/></td>
				      </tr>
					  <tr>
                        <td  align="right" class="tabela1_linha2">Situação</td>
					    <td  align="left" class="tabela1_linha2">Ativo
					      <input name="status" type="radio" value="1"  <?php  if ( $funcionamento->status == 1) { ?> checked="checked" <?php  }?>/>
					      Inativo
					      <input name="status" type="radio" value="0"   <?php  if ( $funcionamento->status == 0) { ?>checked="checked"<?php  }?> /></td>
				      </tr>
					  <tr>
			   ...
			   <input type="hidden"  name="id" value="<?php=stripslashes($funcionamento->id)?>" />
		    </form> 
              <br />
              </p>
          </td>
