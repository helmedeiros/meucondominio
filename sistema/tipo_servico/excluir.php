...
//verifica se o su possue permissão para excluir(3) no modulo(28)
if(!permissoesDAO::temPermissao(28,3,$usuario->id_tipo_usuario)){
...
if ( isset($_POST['id']) && is_numeric($_POST['id']) )
	tiposservicosDAO::delete($_POST['id']);
...
if(!isset($nxt)||($nxt<0))
	...
	$total = tiposservicosDAO::countAll();
	

    $tiposServicos = tiposservicosDAO::findTop($nxt,$total_mural);
            ...
            <table cellpadding="0" cellspacing="0" width="96%" class="tabela1">
                <tr>
                  <td bgcolor="#6598CB"><table width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="89%" class="tabela1_titulo1">Top 10  Tipos de Serviços(ordem alfabética) </td>
                ...
                <tr>
                  <td><table cellpadding="1" cellspacing="1" width="100%">
                      <tr>
                        <td width="28%" class="tabela1_titulo2">Tipo de Usuário </td>
                        <td class="tabela1_titulo2">Descrição</td>
                        <td width="7%" class="tabela1_titulo2">&nbsp;</td>
                      </tr>
					    <?php 
							for ($i = 0; $i < sizeof($tiposServicos); $i++){
								if ($tiposServicos[$i]->status == 0) { $classe = "tabela1_linhaWarning"; } else { $classe = "tabela1_linha2"; }
						?>
                      <tr>
                        <td height="31" nowrap="nowrap" class="<?php=$classe?>"><a href="#"><?php=$tiposServicos[$i]->nome?></a></td>
                        <td nowrap="nowrap" class="<?php=$classe?>"><a href="#">
                        <?php=substr($tiposServicos[$i]->descricao, 0, 50)?><?php  if(strlen($tiposServicos[$i]->descricao) >= 50){?>...<?php  }?></a></td>
                        <td class="<?php=$classe?>" nowrap="nowrap"> <form action="excluir.php" method="post" onSubmit="javascript:return confirma('<?php=$tiposServicos[$i]->nome?>','Tipos de Serviço')"><input type="image" src="../images/xis.jpg" width="20" height="21" border="0" /><input type="hidden" value="<?php=$tiposServicos[$i]->id?>" name="id" /></form></td>
                      </tr>
					  	 <?php 
							}
						?>
