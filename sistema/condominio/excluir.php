...
//verifica se o su possue permissão para excluir(3) no condominio(18)
if(!permissoesDAO::temPermissao(18,3,$usuario->id_tipo_usuario)){
...
if ( isset($_POST['id']) && is_numeric($_POST['id']) )
	condominiosDAO::delete($_POST['id']);
...
if(!isset($nxt)||($nxt<0))
    ...
    $total = condominiosDAO::countAll();
	$condominios = condominiosDAO::findTop($nxt,$total_mural);
        ...
        <tr>
          ...
          <td width="481" bgcolor="#FFFFFF"><!-- texto sempre dentro da tag "<p>" -->
			...
			<?php 
				if ($condominios){
			?>
            <table cellpadding="0" cellspacing="0" width="452" class="tabela1">
                <tr>
                  <td width="450" bgcolor="#6598CB"><table width="100%" cellpadding="0" cellspacing="0">
                      <tr>
                        <td width="89%" class="tabela1_titulo1">Top 10  Condomínios (ordem alfabética) </td>
                        <td width="11%"><a href="home.php"><img src="../images/listar.jpg" width="48" height="17" border="0" /></a></td>
                      </tr>
                  </table></td>
                </tr>
                <tr>
                  <td><table cellpadding="1" cellspacing="1" width="450">
                      <tr>
                        <td width="92" class="tabela1_titulo2">Nome</td>
                        <td width="97" class="tabela1_titulo2">Contato</td>
                        <td width="111" class="tabela1_titulo2">Telefone</td>
                        <td width="101" class="tabela1_titulo2">Responsável</td>
					    ...
					    <?php 
							...
							$contato = contatosDAO::findByPk($condominios[$i]->id_contato);
							if ($condominios[$i]->status == 0) { $classe = "tabela1_linhaWarning"; } else { $classe = "tabela1_linha2"; }
						?>
                      <tr>
                        <td height="31"  class="<?php=$classe?>"><a href="#">
                          <?php=$condominios[$i]->nome?>
                        </a></td>
                        <td class="<?php=$classe?>"><a href="#"><?php=$contato->nome?></a></td>
                        <td class="<?php=$classe?>"><a href="#"><?php=$condominios[$i]->telefone?></a></td>
                        <td class="<?php=$classe?>"><a href="#"><?php=$superusuario->nome?></a></td>
                        <td class="<?php=$classe?>"> <form action="excluir.php" method="post" onSubmit="javascript:return confirma('<?php=$condominios[$i]->nome?>','Condomínio')"><input type="image" src="../images/xis.jpg" width="20" height="21" border="0" /><input type="hidden" value="<?php=$condominios[$i]->id?>" name="id" /></form></td>
                      </tr>
					  	 <?php 
							}
						?>
