<?php 
class publico_alvoDAO{
       function findALL(){
              $return = false;
              $rs = mysql_query("SELECT * FROM publico_alvo WHERE situacao = 1");
  ...
  return $result;
       ...
       function findByPk($id){
              $return = false;
              $rs = mysql_query("SELECT * FROM publico_alvo WHERE FKid_atividades = $id AND situacao = 1");
              while ( $r = mysql_fetch_array($rs) ){
    ...
    $publico_alvo->id_tipo_funcionario = $r['FKid_tipo_usuariosCol'];
        ...
        function countAllByAtv($id){
              $return = false;
              $rs = mysql_query("SELECT COUNT( * ) AS total FROM publico_alvo WHERE FKid_atividadesCol = $id AND situacao = 1");
 ...
 */  
	   ...
	   $result = "";
	  	$rs = mysql_query("SELECT FKid_tipos_funcionariosCol FROM publico_alvo WHERE FKid_atividadesCol = $id");
		while($r = mysql_fetch_array($rs)){
	   ...
	   $result = "";
	  	$rs = mysql_query("SELECT FKid_tipo_usuariosCol FROM publico_alvo WHERE FKid_atividadesCol = $id");
		while($r = mysql_fetch_array($rs)){
		      if ($r['FKid_tipo_usuariosCol'] != 0) {
			  if ($result == "") { 
			   $result = $r['FKid_tipo_usuariosCol'];} else {
		  	  $result = $result.", ".$r['FKid_tipo_usuariosCol']; } }
			 			}
		return $result;
	} 
       ...
       function saveUsr($id, $id_membro){
	 	 return mysql_query("INSERT INTO publico_alvo (FKid_atividadesCol, FKid_tipo_usuariosCol, situacao) VALUES ('{$id}', '{$id_membro}', 1)");
}
