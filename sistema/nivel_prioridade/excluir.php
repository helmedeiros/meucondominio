...
//verifica se o su possue permissão para excluir(3) no modulo(16)
if(!permissoesDAO::temPermissao(16,3,$usuario->id_tipo_usuario)){
...
if ( isset($_POST['id']) && is_numeric($_POST['id']) )
	niveisprioridadeDAO::delete($_POST['id']);
...
if(!isset($nxt)||($nxt<0))
	...
	$total = niveisprioridadeDAO::countAll();
	

    $niveisprioridade = niveisprioridadeDAO::findTop($nxt,$total_mural);
            ...
            <table cellpadding="0" cellspacing="0" width="96%" class="tabela1">
                ...
                <tr>
                  <td><table cellpadding="1" cellspacing="1" width="100%">
                      <tr>
                        <td width="28%" class="tabela1_titulo2">Nível de Prioridade </td>
                        <td class="tabela1_titulo2">Descrição</td>
                        <td width="7%" class="tabela1_titulo2">&nbsp;</td>
                      </tr>
					    <?php 
							for ($i = 0; $i < sizeof($niveisprioridade); $i++){
						?>
                      <tr>
                        <td height="31" nowrap="nowrap" class="tabela1_linha2"><a href="#"><?php=$niveisprioridade[$i]->nome?></a></td>
                        <td nowrap="nowrap" class="tabela1_linha2"><a href="#">
                        <?php=substr($niveisprioridade[$i]->descricao, 0, 50)?><?php  if(strlen($niveisprioridade[$i]->descricao) >= 50){?>...<?php  }?></a></td>
                        <td class="tabela1_linha2" nowrap="nowrap"> <form action="excluir.php" method="post" onSubmit="javascript:return confirma('<?php=$niveisprioridade[$i]->nome?>','Nível de Prioridade')"><input type="image" src="../images/xis.jpg" width="20" height="21" border="0" /><input type="hidden" value="<?php=$niveisprioridade[$i]->id?>" name="id" /></form></td>
                      </tr>
					  	 <?php 
							}
						?>
