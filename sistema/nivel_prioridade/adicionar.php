...
$nivelprioridade = new niveisprioridade();

if ( isset($_POST['nome']) ){
	
	//verifica se o su possue permissão para alterar(1) no modulo(16)
	if(!permissoesDAO::temPermissao(16,1,$usuario->id_tipo_usuario)){	
		header("Location: ../index.php");
		exit();
	}		
	
	//atribui o id ao objeto nivelprioridade
	$nivelprioridade->id = addslashes($_POST['id']);
	$nivelprioridade->nome = addslashes($_POST['nome']);
	$nivelprioridade->descricao = addslashes($_POST['descricao']);
	
	//verifica se o o nome da esta em branco
	if($nivelprioridade->nome != ""){
	
	//verifica se o id recebido aponta para um novo usuário(0) ou um antigo(!=0)
	if ($nivelprioridade->id != 0){
		$nivelprioridade = niveisprioridadeDAO::findByPk($nivelprioridade->id);
	}			
	$nivelprioridade->nome = addslashes($_POST['nome']);
	$nivelprioridade->descricao = addslashes($_POST['descricao']);
	$nivelprioridade->status = 1;
	$id = niveisprioridadeDAO::save($nivelprioridade);
	header("Location: home.php");
	exit();		
	
	}else{
		$msg = "O nome do nível de prioridade não pode estar vazio";
	}
}  

//verifica se a pagina recebeu um id para alteração
if(isset($_GET['id'])){
	$nivelprioridade = niveisprioridadeDAO::findByPk($_GET['id']);
	$ep = 0;
}else{
	//verifica se o su possue permissão para inserir(2) no modulo(16)
	if(!permissoesDAO::temPermissao(16,2,$usuario->id_tipo_usuario)){
		header("Location: ../index.php");
		exit();
	}
}
      ...
      <table width="545" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td width="481" bgcolor="#FFFFFF"><!-- texto sempre dentro da tag "<p>" -->
            ...
            <br />
            <br />
			<form action="adicionar.php" method="post" onSubmit="javascript:return confirmaTipoUser(this)" name="nivelprioridade" id="nivelprioridade">
			  <div align="center">
			  <font class="warning">
			    <?php  if(isset($msg) )
						echo $msg;?>
			  ...
			  <table cellpadding="0" cellspacing="0" width="96%" class="tabela1">
                ...
                <tr>
                  <td><table cellpadding="1" cellspacing="1" width="100%">
                      ...
                      <tr>
						...
						<td class="tabela1_linha2" ><input type="text" class="FORMULARIO"  name="nome" size="30" maxlength="30" value="<?php=stripslashes($nivelprioridade->nome)?>"></td>
					  </tr>
					  <tr>
                        <td class="tabela1_linha2"  align="right"> Descrição </td>
					    <td class="tabela1_linha2" ><textarea name="descricao" cols="50" rows="2" class="FORMULARIO" id="descricao"><?php=stripslashes($nivelprioridade->descricao)?></textarea></td>
				      </tr>
					  <tr>
			   ...
			   <input type="hidden"  name="id" value="<?php=stripslashes($nivelprioridade->id)?>" />
		    </form> 
              <br />
              </p>
          </td>
