...
//verifica se o su possue permissão para excluir(3) no modulo(4)
if(!permissoesDAO::temPermissao(4,3,$usuario->id_tipo_usuario)){
...
if ( isset($_POST['id']) && is_numeric($_POST['id']) )
	tiposusuariosDAO::delete($_POST['id']);
...
if(!isset($nxt)||($nxt<0))
	...
	$total = tiposusuariosDAO::countAll();
	

    $tiposUsuarios = tiposusuariosDAO::findTop($nxt,$total_mural);
                ...
                <tr>
                  <td><table cellpadding="1" cellspacing="1" width="100%">
                      <tr>
					    ...
					    <?php 
							for ($i = 0; $i < sizeof($tiposUsuarios); $i++){
								if ($tiposUsuarios[$i]->status == 0) { $classe = "tabela1_linhaWarning"; } else { $classe = "tabela1_linha2"; }
						?>
                      <tr>
                        <td class="<?php=$classe?>" nowrap="nowrap"><a href="#">
                          <?php=$tiposUsuarios[$i]->id?>
                        </a></td>
                        <td class="<?php=$classe?>" nowrap="nowrap"><a href="#">
                          <?php=$tiposUsuarios[$i]->nome?>
                        </a></td>
                        <td nowrap="nowrap" class="<?php=$classe?>"><a href="#">
                          <?php=substr($tiposUsuarios[$i]->descricao, 0, 40)?>
                          <?php  if(strlen($tiposUsuarios[$i]->descricao) >= 40){?>
                          ...
                          <?php  }?>
                        </a></td>
                        <td class="<?php=$classe?>" nowrap="nowrap"> <form action="excluir.php" method="post" onSubmit="javascript:return confirma('<?php=$tiposUsuarios[$i]->nome?>','Tipo de Usuário')"><input type="image" src="../images/xis.jpg" width="20" height="21" border="0" /><input type="hidden" value="<?php=$tiposUsuarios[$i]->id?>" name="id" /></form></td>
                      </tr>
					  	<?php 
							}
						?>
