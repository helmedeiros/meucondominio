...
if ( isset($_POST['nome']) ){
	...
	//atribui o id ao objeto tipousuario
	$tipousuario->id = addslashes($_POST['id']);
	$tipousuario->nome = addslashes($_POST['nome']);
	$tipousuario->descricao = addslashes($_POST['descricao']);
	...
	if ($tipousuario->id != 0){
		$tipousuario = tiposusuariosDAO::findByPk($tipousuario->id);
	}			
	$tipousuario->nome = addslashes($_POST['nome']);
	$tipousuario->descricao = addslashes($_POST['descricao']);
	$tipousuario->status = addslashes($_POST['status']);
	$id = tiposusuariosDAO::save($tipousuario);
	header("Location: home.php");
	exit();	
	
	}else{
		$msg = "O nome do Tipo de usuário não pode estar vazio";
	}		
}  

//verifica se a pagina recebeu um id para alteração
if(isset($_GET['id'])){
	$tipousuario = tiposusuariosDAO::findByPk($_GET['id']);
	$ep = 0;
}else{
      ...
      <table width="545" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td width="481" bgcolor="#FFFFFF"><!-- texto sempre dentro da tag "<p>" -->
            ...
            <br />
            <br />
			<form action="adicionar.php" method="post" onSubmit="javascript:return confirmaTipoUser(this)" name="tipousuario" id="tipousuario">
			  <div align="center">
			  <font class="warning">
			    <?php  if(isset($msg) )
						echo $msg;?>
                ...
                <tr>
                  <td><table cellpadding="1" cellspacing="1" width="100%">
                      ...
                      <tr>
						...
						<td class="tabela1_linha2" ><input type="text" class="FORMULARIO"  name="nome" size="18" maxlength="18" value="<?php=stripslashes($tipousuario->nome)?>"></td>
					  </tr>
					  <tr>
                        <td class="tabela1_linha2"  align="right"> Descrição </td>
					    <td class="tabela1_linha2" ><textarea name="descricao" cols="50" rows="2" class="FORMULARIO" id="descricao"><?php=stripslashes($tipousuario->descricao)?></textarea></td>
				      </tr>
					  <tr>
					    <td  align="right" class="tabela1_linha2">Situação</td>
				        <td  align="right" class="tabela1_linha2"><div align="left">Ativo
				          <input name="status" type="radio" value="1"  <?php  if ( $tipousuario->status == 1) { ?> checked="checked" <?php  }?>/>
				          Inativo
  <input name="status" type="radio" value="0"   <?php  if ( $tipousuario->status == 0) { ?>checked="checked"<?php  }?> />
                        </div></td>
					  </tr>
					  <tr>
			   ...
			   <input type="hidden"  name="id" value="<?php=stripslashes($tipousuario->id)?>" />
		    </form> 
              <br />
              </p>
          </td>
